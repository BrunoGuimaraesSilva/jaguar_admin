		...
		<tbody>
			...
			while ($dados = $consulta->fetch(PDO::FETCH_OBJ)) {
				if ($dados->status == "P") {
					...
					$status = '<span class="badge badge-error">Devolvido<span>';
				} else if ($dados->status == "E") {
					$status = '<span class="badge badge-info">Extraviado<span>';
				} else if ($dados->status == "A") {
					$status = '<span class="badge badge-primary">Ag pagamento<span>';
				} else if ($dados->status == "T") {
					$status = '<span class="badge badge-primary">Troca<span>';
				}
			?>
				<tr>
					<td><?= $dados->id ?></td>
					...
					<td><?= $dados->data ?></td>
					<td><?= $status ?></td>
					<td class="text-center">R$ <?= getTotalVenda($pdo,$dados->id)?></td>
				</tr>
			<?php
			}
			?>
