...
function getTotalVenda($pdo, $idVenda)
{

	$sql = "SELECT sum(valor * quantidade) AS total FROM venda_produto WHERE venda_id = :idVenda LIMIT 1";
	$consulta = $pdo->prepare($sql);
	$consulta->bindParam(":idVenda", $idVenda);
	$consulta->execute();
